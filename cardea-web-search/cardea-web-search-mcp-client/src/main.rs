...
const SOCKET_ADDR: &str = "127.0.0.1:8010";
...
#[derive(Parser, Debug)]
#[command(author, version, about = "Cardea Web Search MCP client")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.transport {
        TransportType::Sse => {
            ...
            tracing::info!(
                "Connecting to Cardea Web Search MCP server via sse: {}",
                url
            );
            ...
            // Call the sum tool
            let web_search_result = mcp_client.peer().call_tool(request_param).await?;

            tracing::info!(
                "Web search result:\n{}",
                serde_json::to_string_pretty(&web_search_result)?
            );
        ...
        TransportType::StreamHttp => {
            ...
            tracing::info!(
                "Connecting to Cardea Web Search MCP server via stream-http: {}",
                url
            );
