...
#[derive(Parser, Debug)]
#[command(author, version, about = "Cardea TiDB MCP client")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.transport {
        TransportType::Sse => {
            ...
            tracing::info!("Connecting to Cardea TiDB MCP server via sse: {}", url);
        ...
        TransportType::StreamHttp => {
            ...
            tracing::info!(
                "Connecting to Cardea TiDB MCP server via stream-http: {}",
                url
            );
