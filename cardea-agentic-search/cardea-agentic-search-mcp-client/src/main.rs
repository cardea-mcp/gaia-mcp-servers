...
#[derive(Parser, Debug)]
#[command(author, version, about = "Cardea Agentic Search MCP client")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.transport {
        TransportType::StreamHttp => {
            ...
            tracing::info!(
                "Connecting to Cardea Agentic Search MCP server via stream-http: {}",
                url
            );
        ...
        TransportType::Sse => {
            ...
            tracing::info!(
                "Connecting to Cardea Agentic Search MCP server via sse: {}",
                url
            );
